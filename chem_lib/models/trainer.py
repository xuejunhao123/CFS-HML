...
class Meta_Trainer(nn.Module):
    ...
    def get_adaptable_weights(self, model, adapt_weight = None):
        ...
        fenc  = lambda x: x[0] == 'mol_encoder' # 若参数名以mol_encoder为开头则返回TURE，本质上为筛选gnn参数：W_g
        frel  = lambda x: x[0] == 'adapt_relation'
        fedge = lambda x: x[0] == 'adapt_relation' and 'edge_layer'  in x[1] # W_a
        fnode = lambda x: x[0] == 'adapt_relation' and 'node_layer'  in x[1] # W_r
        fclf  = lambda x: x[0] == 'adapt_relation' and 'fc'  in x[1] # 筛选最后一层全连接网络的参数，本质上为筛选classifier参数: W_c
        
        fkernel = lambda x: x[0] == 'adapt_relation' and ('kernel'  in x[1]) # 新加的核函数的参数，也属于W_a
        fproperty = lambda x: x[0] == 'encode_projection' # 筛选出与property-aware embeddings有关的参数: W_p
        ...
        if adapt_weight==0:
            ...
            flag=lambda x: not (fenc(x) or fnode(x) or fedge(x) or frel(x))
        elif adapt_weight==8:
            flag=lambda x: not (fenc(x) or fclf(x) or frel(x))
        elif adapt_weight==9:
            flag=lambda x: not (fenc(x) or fclf(x) or frel(x) or fedge(x))
        elif adapt_weight==10:
            flag=lambda x: not (fnode(x) or fclf(x) or frel(x) or fedge(x))
        elif adapt_weight==11:
            flag=lambda x: not (fproperty(x) or fnode(x) or fedge(x))
        elif adapt_weight==12:
            flag=lambda x: not (fproperty(x) or fnode(x) or fedge(x) or fkernel(x))
        elif adapt_weight==13:
            flag=lambda x:  (fkernel(x))
        else:
            flag= lambda x: True
        ...
        if self.train_epoch < self.args.meta_warm_step or self.train_epoch>self.args.meta_warm_step2:
            ...
            adaptable_weights = []
            adaptable_names=[]
            for name, p in model.module.named_parameters(): # 原来总共61参数
                names=name.split('.')
                if flag(names):
    ...
    def train_step(self):
        ...
        for k in range(self.update_step):
            ...
            for task_id in task_id_list:
                ...
                model = self.model.clone() # 克隆一个模型，用于内循环第一次梯度更新
                model.train()
                ...
                for inner_step in range(self.inner_update_step):
                    ...
                    loss_adapt = self.get_loss(model, train_data, pred_adapt, train=True, flag = 1) # s_data
                    model.adapt(loss_adapt, adaptable_weights = adaptable_weights) # 更新内部参数

                pred_eval = self.get_prediction(model, train_data, train=True)
                loss_eval = self.get_loss(model, train_data, pred_eval, train=True, flag = 0) ## s_data and q_data
